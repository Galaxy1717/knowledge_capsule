...
with open("data/knowledge_capsule_mapping_esb.json", "r") as f:
    data_esb = json.load(f)
...
esb_references = list(data_esb.keys())
...
if demo == "Test Capsules ESB":
    # Streamlit app
    st.title(cross_string("Adaptive v2"))
    st.title("Création de capsule de connaissance")

    # Dropdown to select reference
    esb_selected_reference = st.selectbox("Choix de la référence:", esb_references)

    concept_keys = ["label", "definition"]
    exercise_keys = ["id", "question", "query", "statement", "answers", "distractors", "veracity", "feedback"]

    # Display selected reference data
    if esb_selected_reference:
        data = data_esb
        st.header("Introduction")
        st.write(data[esb_selected_reference]["introduction"])
        st.header("Synthèse d'information")
        st.write(data[esb_selected_reference]["synthesis"])

        st.header("Concepts important")
        concept_collection = data[esb_selected_reference]["concepts"]
        new_collection = []
        for concept_dict in concept_collection:
            new_dic = {}
            if concept_dict["score"] >= 8:
        ...
        st.header("Je teste mes connaissances")
        exercise_collection = data[esb_selected_reference]["exercises"]
        new_collection = []
        for exercise_dict in exercise_collection:
            new_dic = {}
            if exercise_dict["score"] >= 7:
        ...
        st.header("Sources")
        relevant_sources = data[esb_selected_reference]["sources"]
        new_collection = []
        for source_dict in relevant_sources:
            new_dic = {}
            new_dic["name"] = source_dict["name"]
        ...
        st.header("Visualisation du sous-graphe")
        if st.button("Visualiser le sous-graphe de connaissance"):
            html_path = f"data/graph_esb/graph_capsule_esb_{esb_selected_reference}.html"
            if f"graph_capsule_esb_{esb_selected_reference}.html" not in os.listdir("data/graph_esb/"):
                html_path = f"data/graph_esb/graph_capsule_esb_{esb_selected_reference[:20]}.html"
            HtmlFile = open(html_path, 'r', encoding='utf-8')
            # Load HTML file in HTML component for display on Streamlit page
            components.html(HtmlFile.read(), height=1000, width=1000)
